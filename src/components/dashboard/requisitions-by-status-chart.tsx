...
interface RequisitionsByStatusChartProps {
  filters?: { month?: string; year?: string }; // Filters might be relevant later
}

export function RequisitionsByStatusChart({ filters }: RequisitionsByStatusChartProps) {
  ...
  return (
    <Card className="shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-in-out">
      <CardHeader>
        <CardTitle className="font-headline">Requisitions by Status</CardTitle>
        <CardDescription>Distribution of purchase requisitions by their current status.</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <div className="flex flex-col justify-center items-center h-[300px] text-destructive p-4 border border-destructive/50 bg-destructive/10 rounded-md">
           ...
           <div className="flex flex-col justify-center items-center h-[300px]">
            <p className="text-muted-foreground">No requisition data available for the selected period.</p>
          </div>
        ) : (
