...
"[project]/.next-internal/server/app/api/quotes/[id]/route/actions.js [app-rsc] (server actions loader, ecmascript)": ((__turbopack_context__) => {
"use strict";
...
"[project]/src/app/api/quotes/[id]/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
async function GET(request, { params }) {
    ...
    try {
      ...
      WHERE q.id = ?
    `;
        const [quoteRows] = await connection.execute(quoteQuery, [
            quoteId
        ]);
        ...
        const [itemRows] = await connection.execute('SELECT * FROM QuoteItem WHERE quoteId = ?', [
            quoteId
        ]);
        quoteData.items = itemRows.map((item)=>({
                ...item,
                quantity: parseInt(item.quantity, 10),
                unitPrice: parseFloat(item.unitPrice)
            }));
        console.log(`[API_INFO] /api/quotes/${quoteId} GET: Successfully fetched quote with ${quoteData.items.length} items.`);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(quoteData);
    } catch (error) {
        console.error(`[API_ERROR] /api/quotes/${quoteId} GET:`, error);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: `Failed to fetch quote with ID ${quoteId}.`,
...
async function PUT(request, { params }) {
    ...
    try {
        ...
        const [existingQuote] = await connection.execute('SELECT status FROM Quote WHERE id = ?', [
            quoteId
        ]);
        if (existingQuote.length === 0) {
            await connection.rollback();
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                error: `Quote with ID ${quoteId} not found for update.`
            }, {
                status: 404
            });
        }
        ...
        await connection.execute(`UPDATE Quote SET 
       ...
       WHERE id = ?`, [
        ...
        await connection.execute('DELETE FROM QuoteItem WHERE quoteId = ?', [
            quoteId
        ]);
      ...
      WHERE q.id = ?
    `;
        const [updatedQuoteRows] = await connection.execute(getUpdatedQuery, [
            quoteId
        ]);
        const updatedQuoteData = updatedQuoteRows[0];
        const [updatedItemRows] = await connection.execute('SELECT * FROM QuoteItem WHERE quoteId = ?', [
            quoteId
        ]);
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(updatedQuoteData);
    } catch (error) {
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: `Failed to update quote with ID ${quoteId}.`,
            details: error.message,
            code: error.code
        }, {
            status: 500
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e4d37637._.js.map